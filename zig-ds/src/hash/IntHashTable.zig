...
pub fn IntHashTable(comptime T: type) type {
    return struct {
        const Self = @This();

        hash_table: ds.HashTable(i64, T),

        pub fn init(allocator: *Allocator) Self {
            return .{
                .hash_table = ds.HashTable(i64, T).init(allocator),
            };
        }
        ...
        pub fn put(self: *Self, key: i64, value: T) !void {
            return self.hash_table.put(key, value);
        }

        pub fn get(self: *Self, key: i64) ?T {
        ...
        pub fn remove(self: *Self, key: i64) ?T {
        ...
        pub fn iterator(self: *Self) ds.HashTable(i64, T).Iterator {
...
test "IntHashTable" {
    ...
    var table = IntHashTable(u32).init(allocator);
    defer table.deinit();

    try table.put(1, 100);
    try table.put(2, 200);

    try std.testing.expectEqual(@as(?u32, 100), table.get(1));
    try std.testing.expectEqual(@as(?u32, 200), table.get(2));
